...
- `8月14日-6` 在统一世界状态上继续加入世界钟同步：读取日程时自动推进角色当前位置，NPC 按夜间/工作时段/常去地点切换当前地点，记录移动起点、终点和预计分钟数；聊天和家园继续读取同一份状态。聊天提示词已改为只注入当前相关地点/NPC，并按角色当地时区计算 NPC 作息。地图/NPC相关测试通过，生产构建已完成。
